...
# 計測モード(省略時 "first_passage"。詳細は「計測モード」の節を参照)
mode = "first_passage"
...
- **`first_passage`(既定)**: 各試行を、重心が初期位置から1周期分(x₀ ± L)進んだ時点で打ち切ります。初通過時間の平均 T₁ とその2乗平均 T₂ から μ と D_eff を求めます。時間 T までに未通過だった試行は打ち切られ、平均の計算から除外されます。
- **`fixed_time`**: 全試行を時間 T まで走らせ、各試行の変位 Δx = x(T) − x₀ の統計から次の量を直接算出します。
