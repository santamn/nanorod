...
__device__ double omega_device(double x) {
    const double two_pi = 6.283185307179586476925286766559;
    double a = two_pi * x;
    return sin(a) + 0.25 * sin(2.0 * a) + 1.12;
}
...
extern "C" __global__ void simulate_kernel(
    ...
    for (unsigned int local_step = 0; local_step < steps_per_launch; ++local_step) {
        ...
        double cos2 = cos(2.0 * phii);
        double sin2 = sin(2.0 * phii);
        ...
        double dphi = params.d_r * (torque_sum + tau_e) * params.dt
            + sqrt(2.0 * params.d_r * params.dt) * normal_b.x;
